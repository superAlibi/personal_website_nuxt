...
const hobbys = getHobbys();
const projects = getSharedProjects();
const { baseSkill, frameworkLevel, } = getSkillLevel();

const hobbyItems: HobbyItem[] = hobbys.map<HobbyItem>((item) => {
  return {
    title: {
      children: item.text,
    },
  ...
  <div class="xl:max-w-7xl mx-auto px-4">
    ...
    <div class="mt-8">
      ...
      <div class="text-wrap flex gap-4   flex-wrap justify-center max-w-5xl mx-auto">
        ...
        <span v-for="skill of frameworkLevel"
          :class="randomBadgeStyle.at(Math.floor(Math.random() * randomBadgeStyle.length))">
          {{ skill.text }}
        </span>
